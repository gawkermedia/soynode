...
SoyCompiler.prototype.compileTemplateFiles = function (files, callback) {
  ...
  var inputDir = this._options.inputDir
  var self = this
  this._maybeUsePrecompiledFiles(outputDir, files)
    .then(function (dirtyFiles) {
...
SoyCompiler.prototype._compileTemplateFilesAsync = function (inputDir, outputDir, allFiles, dirtyFiles) {
  ...
  var pathToSoyJar = options.soyJarDir

  // Arguments for running the soy compiler via java.
  var args = [
    '-classpath', [ pathToSoyJar ].concat(options.classpath).join(path.delimiter),
  ...
  return runCompiler().then(function () {
    ...
    return next().fail(function (err) {
      debug('Error post-processing templates', err)
      throw err
    })
  })
}
...
SoyCompiler.prototype._finalizeCompileTemplates = function (outputDir, emitter) {
  ...
  if (this._options.eraseTemporaryFiles && !this._options.allowDynamicRecompile) {
    exec('rm -r \'' + outputDir + '\'', {}, function (err) {
      ...
      if (err) debug('Error deleting temporary files', err)
    })
  }
}
...
SoyCompiler.prototype._maybeUsePrecompiledFiles = function (outputDir, files) {
  var precompiledDir = this._options.precompiledDir
  if (!precompiledDir) {
  ...
  return Q.resolve(true)
    .then(function () {
      ...
      return Q.all(files.map(function (file) {
        return self._preparePrecompiledFile(outputDir, precompiledDir, file, vmTypes)
          .then(function (ok) {
            return ok ? '' : file
          })
      }))
    ...
    .then(function (dirtyFiles) {
      ...
      if (dirtyFiles.length != files.length) {
        debug('Loaded %s precompiled files', files.length - dirtyFiles.length)
      }
      return dirtyFiles
    })
    .fail(function (err) {
      debug('Failed loading precompiled files', err)
      return files
    })
}
...
SoyCompiler.prototype._preparePrecompiledFile = function (outputDir, precompiledDir, file, vmTypes) {
  var self = this
  var precompiledFilesOkPromise = Q.all(vmTypes.map(function (vmType) {
    var precompiledFileName = self._getOutputFile(precompiledDir, file, vmType)
    var outputFileName = self._getOutputFile(outputDir, file, vmType)

    var precompiledFileOkPromise = Q.nfcall(fs.stat, precompiledFileName)
      .then(function (exists) {
        ...
        if (outputFileName != precompiledFileName) {
          return Q.nfcall(fs.mkdirs, path.dirname(outputFileName))
            .then(function () {
              return Q.nfcall(copy, precompiledFileName, outputFileName)
            })
            .then(function () {
              return true
            })
        ...
        return false // stat is expected to error out if the file isn't there.
      })
...
SoyCompiler.prototype._postCompileProcess = function (outputDir, files, vmType) {
  ...
  try {
    ...
    debug('Error concatenating files', e)
  }
